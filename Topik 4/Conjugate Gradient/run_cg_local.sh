#!/bin/bash
# Script to run Conjugate Gradient method on a local system (AMD Ryzen 7 5800H - 8 cores/16 threads)
...
RESULTS_DIR="conjugate_gradient_results_local"
mkdir -p "$RESULTS_DIR"

# Create CSV file for results
RESULTS_CSV="$RESULTS_DIR/conjugate_gradient_results_local.csv"
echo "N,np,ComputeTime,CommunicationTime,TotalTime,Iterations" > "$RESULTS_CSV"

# Summary file
SUMMARY_FILE="$RESULTS_DIR/summary.txt"
echo "Conjugate Gradient Method - Local PC Environment Results" > "$SUMMARY_FILE"
echo "=============================================" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
for N in "${SIZES[@]}"; do
    echo "====================================================="
    echo "Running Conjugate Gradient solver on local PC with N=$N"
    echo "====================================================="
