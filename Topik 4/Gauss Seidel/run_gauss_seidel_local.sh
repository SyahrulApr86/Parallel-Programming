#!/bin/bash
# Script to run Gauss-Seidel iteration on a local system (AMD Ryzen 7 5800H - 8 cores/16 threads)
...
RESULTS_DIR="gauss_seidel_results_local"
mkdir -p "$RESULTS_DIR"

# Create CSV file for results
RESULTS_CSV="$RESULTS_DIR/gauss_seidel_results_local.csv"
echo "N,np,ComputeTime,CommunicationTime,TotalTime,Iterations" > "$RESULTS_CSV"

# Summary file
SUMMARY_FILE="$RESULTS_DIR/summary.txt"
echo "Gauss-Seidel Iteration - Local PC Environment Results" > "$SUMMARY_FILE"
echo "=================================================" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
for N in "${SIZES[@]}"; do
    echo "====================================================="
    echo "Running Gauss-Seidel solver on local PC with N=$N"
    echo "====================================================="
