#!/bin/bash
#SBATCH --job-name=matmat-cluster
#SBATCH --partition=batch
#SBATCH --nodes=1-8        # Request up to 8 nodes
#SBATCH --output=matmat_cluster_%j.out
#SBATCH --error=matmat_cluster_%j.err
...
RESULTS_DIR="matmat_results_cluster"
mkdir -p "$RESULTS_DIR"

# Create CSV file for results
RESULTS_CSV="$RESULTS_DIR/matmat_results_cluster.csv"
echo "Size,np,ComputeTime,CommunicationTime,TotalTime" > "$RESULTS_CSV"

# Summary file
SUMMARY_FILE="$RESULTS_DIR/summary.txt"
echo "Matrix-Matrix Multiplication - Cluster Environment Results" > "$SUMMARY_FILE"
echo "==========================================" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
for N in "${SIZES[@]}"; do
    ...
    for np in "${PROCS[@]}"; do
        ...
        result=$(run_with_timeout "mpirun --mca btl_tcp_if_exclude docker0,lo -np $np ./matmat $N")
