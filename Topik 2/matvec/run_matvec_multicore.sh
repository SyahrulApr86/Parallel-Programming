#!/bin/bash
# Script to run Matrix-Vector multiplication on a multicore system (1 CPU with 64 cores)
...
RESULTS_DIR="matvec_results_multicore"
mkdir -p "$RESULTS_DIR"

# Create CSV file for results
RESULTS_CSV="$RESULTS_DIR/matvec_results_multicore.csv"
echo "Size,np,ComputeTime,CommunicationTime,TotalTime" > "$RESULTS_CSV"

# Summary file
SUMMARY_FILE="$RESULTS_DIR/summary.txt"
echo "Matrix-Vector Multiplication - Multicore Environment Results" > "$SUMMARY_FILE"
echo "============================================" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
