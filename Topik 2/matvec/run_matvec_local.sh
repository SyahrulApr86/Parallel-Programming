#!/bin/bash
# Script to run Matrix-Vector multiplication on a local system (AMD Ryzen 7 5800H - 8 cores/16 threads)
...
RESULTS_DIR="matvec_results_local"
mkdir -p "$RESULTS_DIR"

# Create CSV file for results
RESULTS_CSV="$RESULTS_DIR/matvec_results_local.csv"
echo "Size,np,ComputeTime,CommunicationTime,TotalTime" > "$RESULTS_CSV"

# Summary file
SUMMARY_FILE="$RESULTS_DIR/summary.txt"
echo "Matrix-Vector Multiplication - Local PC Environment Results" > "$SUMMARY_FILE"
echo "==========================================" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
for N in "${SIZES[@]}"; do
    ...
    for np in "${PROCS[@]}"; do
        ...
        result=$(run_with_timeout "mpirun -np $np ./matvec $N")
