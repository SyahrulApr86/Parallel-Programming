...
// Kernel CUDA tanpa shared memory
__global__ void matrix_multiply_cuda_simple(float *A, float *B, float *C, int N) {
...
int main(int argc, char *argv[]) {
    ...
    printf("Matrix Multiplication CUDA Simple: N=%d, blockSize=%d, gridSize=%d\n",
    ...
    matrix_multiply_cuda_simple<<<gridDim, blockDim>>>(d_A, d_B, d_C, N);
    ...
    printf("CUDA Simple: Computation Time = %.4f ms, Communication Time = %.4f ms\n",
    ...
    if (save_result) {
        ...
        save_matrix(C, N, "matrix_C_cuda_simple.txt");
    }
