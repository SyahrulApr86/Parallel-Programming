...
void matrix_vector_mult(double *local_A, double *x, double *y, int local_start, int local_rows, int N) {
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        printf("Conjugate Gradient method using MPI\n");
    ...
    for (iter = 0; iter < max_iter && residual_norm > tol; iter++) {
        ...
        matrix_vector_mult(local_A, p, local_Ap, local_start, local_rows, N);
        compute_end = MPI_Wtime();
        compute_time += compute_end - compute_start;
