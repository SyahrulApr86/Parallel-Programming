...
typedef struct {
    int *rowPtr;  // Row pointers (size N+1)
...
// Function to generate a symmetric positive-definite sparse matrix
void generate_spd_sparse_matrix(double *A, double *b, int N, double sparsity) {
    int i, j;
    srand(12345);  // Fixed seed for reproducibility
    
    // First generate a random sparse matrix
    // Initialize to zeros
    for (i = 0; i < N*N; i++) {
        A[i] = 0.0;
    }
    
    // Populate with random values (with desired sparsity)
    for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
...
// Function to compute sparse matrix-vector product y = A*x
void sparse_matrix_vector_mult(CSRMatrix *local_A, double *x, double *y, int local_rows) {
    for (int i = 0; i < local_rows; i++) {
        y[i] = 0.0;
        for (int j = local_A->rowPtr[i]; j < local_A->rowPtr[i + 1]; j++) {
            y[i] += local_A->values[j] * x[local_A->colIdx[j]];
        }
    }
}

// Function to compute dot product of two vectors
double dot_product(double *a, double *b, int local_rows) {
...
double compute_residual(CSRMatrix *A, double *b, double *x, int N) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = A->rowPtr[i]; j < A->rowPtr[i + 1]; j++) {
            residual[i] -= A->values[j] * x[A->colIdx[j]];
        }
...
int main(int argc, char *argv[]) {
    ...
    double sparsity = 0.8;  // Default sparsity level (80% zeros)
    ...
    if (rank == 0) {
        printf("Conjugate Gradient method (Sparse MPI)\n");
        printf("Matrix size: %d x %d\n", N, N);
        printf("Number of processes: %d\n", size);
        printf("Maximum iterations: %d\n", max_iter);
        printf("Tolerance: %e\n", tol);
        printf("Sparsity: %.2f (%.2f%% zeros)\n", sparsity, sparsity * 100.0);
    }
    ...
    double *A_dense = NULL;  // Full matrix (only on rank 0)
    CSRMatrix A_csr;         // CSR format of A (only on rank 0)
    double *b = NULL;       // Full right-hand side (only on rank 0)
    double *x = (double*)malloc(N * sizeof(double));  // Solution vector
    
    // Local portions
    CSRMatrix local_A_csr;    // Local portion of A in CSR format
    ...
    double *local_r = (double*)malloc(local_rows * sizeof(double));
    double *local_Ap = (double*)malloc(local_rows * sizeof(double));
    ...
    if (rank == 0) {
        ...
        if (A_dense == NULL || b == NULL) {
            printf("Memory allocation failed on rank 0\n");
            MPI_Abort(MPI_COMM_WORLD, 1);
            return 1;
        }
        
        generate_spd_sparse_matrix(A_dense, b, N, sparsity);
        dense_to_csr(A_dense, N, &A_csr);
        
        printf("Matrix has %d non-zero elements (%.2f%% sparsity)\n", 
               A_csr.nnz, 100.0 * (1.0 - (double)A_csr.nnz / (N * N)));
    }
    ...
    // We need to scatter the CSR matrix, which is more complex
    // For simplicity, we'll recreate a dense local matrix from the parts,
    // then convert it back to CSR format locally
    double *local_A_dense = (double*)malloc(local_rows * N * sizeof(double));
    ...
    // Scatter dense matrix (for simplicity)
    MPI_Scatterv(A_dense, sendcounts, displs, MPI_DOUBLE, 
                 local_A_dense, local_rows * N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    
    // Convert local part to CSR
    dense_to_csr(local_A_dense, local_rows, &local_A_csr);
    ...
    MPI_Scatterv(b, sendcounts, displs, MPI_DOUBLE,
                 local_b, local_rows, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    comm_start = MPI_Wtime();
    MPI_Allgatherv(local_r, local_rows, MPI_DOUBLE,
    ...
    comm_start = MPI_Wtime();
    double local_r_dot_r = dot_product(local_r, local_r, local_rows);
    ...
    for (iter = 0; iter < max_iter && residual_norm > tol; iter++) {
        // Compute Ap with sparse matrix-vector multiply
        compute_start = MPI_Wtime();
        sparse_matrix_vector_mult(&local_A_csr, p, local_Ap, local_rows);
        compute_end = MPI_Wtime();
        compute_time += compute_end - compute_start;
        
        // Gather all Ap pieces
        comm_start = MPI_Wtime();
        MPI_Allgatherv(local_Ap, local_rows, MPI_DOUBLE,
        ...
        compute_start = MPI_Wtime();
        double local_p_dot_Ap = dot_product(p + local_start, local_Ap, local_rows);
        compute_end = MPI_Wtime();
        compute_time += compute_end - compute_start;
        ...
        compute_start = MPI_Wtime();
        double local_r_dot_r_new = dot_product(local_r, local_r, local_rows);
        compute_end = MPI_Wtime();
        compute_time += compute_end - compute_start;
    ...
    if (rank == 0) {
        ...
        if (b != NULL) {
            double final_residual = compute_residual(&A_csr, b, x, N);
    ...
    // Free CSR memory
    free(local_A_csr.rowPtr);
    free(local_A_csr.colIdx);
    free(local_A_csr.values);
    
    if (rank == 0) {
        if (A_dense != NULL) free(A_dense);
        if (b != NULL) free(b);
        free(A_csr.rowPtr);
        free(A_csr.colIdx);
        free(A_csr.values);
